...
int getsockopt_win(int sockfd, int level, int optname, void *optval, socklen_t *optlen) {
  ...
  return ret == 0 ? *optlen = size, 0 : -1;
}

int setsockopt_win(int sockfd, int level, int optname, const void *optval, socklen_t optlen) {
  if (optname == SO_NOSIGPIPE) {
    // ignore
    return 0;
  }
...
#define getsockopt getsockopt_win
#define setsockopt setsockopt_win
...
#ifdef _MSC_VER    
  if (connect(sock->sockfd, sa, slen) != 0 && WSAGetLastError() != WSAEWOULDBLOCK)
#else
